...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _handleUpdateProfile() async {
    ...
    try {
      if (_currentUser == null) return _showMessage('Please sign in', error: true);
  ...
  Future<void> _showEditDialog(DocumentSnapshot doc) async {
    ...
    final TextEditingController locCtrl = TextEditingController(text: data['location'] ?? '');
    ...
    if (saved == true) {
      try {
        await FirebaseFirestore.instance.collection('problems').doc(doc.id).update({
  ...
  Future<void> _confirmDelete(DocumentSnapshot doc) async {
    ...
    if (confirmed == true) {
      try {
        await FirebaseFirestore.instance.collection('problems').doc(doc.id).delete();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: user == null
          ? Center(child: Padding(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, children: [const Text('Please sign in to view your profile.'), const SizedBox(height: 12), ElevatedButton(onPressed: () => Navigator.pushNamed(context, '/login'), child: const Text('Sign in'))])))
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(onPressed: _handleUpdateProfile, child: const Text('Update Profile')),
                  const SizedBox(height: 8),
                  ...
                  StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (snapshot.hasError) {
                        ...
                        if (err is FirebaseException && (err.code == 'failed-precondition' || (err.message ?? '').toLowerCase().contains('requires an index'))) {
                          return Padding(
                            padding: const EdgeInsets.all(12.0),
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                const Text('This query requires a Firestore composite index.', style: TextStyle(fontWeight: FontWeight.bold)),
                                const SizedBox(height: 8),
                                const Text('To create it: Open Firebase Console → Firestore → Indexes → Add Index, then set:'),
                                const SizedBox(height: 6),
                                const Text('Collection: problems', style: TextStyle(fontWeight: FontWeight.w600)),
                                const Text('Fields: userId (Ascending), createdAt (Descending)'),
                                const SizedBox(height: 12),
                                Row(children: [
                                  ElevatedButton(
                                    onPressed: () {
                                      // Copy a short instruction to clipboard to help the developer create the index
                                      Clipboard.setData(const ClipboardData(text: 'Collection: problems\nFields: userId (Ascending)\ncreatedAt (Descending)'));
                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Index instructions copied to clipboard')));
                                    },
                                    child: const Text('Copy index instructions'),
                                  ),
                                  const SizedBox(width: 10),
                                  ElevatedButton(
                                    onPressed: () {
                                      final url = 'https://console.firebase.google.com/project/fixora-291df/firestore/indexes';
                                      Clipboard.setData(ClipboardData(text: url));
                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Firestore indexes URL copied - paste in browser')));
                                    },
                                    child: const Text('Copy Firestore Console URL'),
                                  ),
                                ]),
                              ],
                            ),
                      ...
                      docs.sort((a, b) {
                        ...
                        else if (aTs is DateTime) aDate = aTs;

                        if (bTs is Timestamp) bDate = bTs.toDate();
                        else if (bTs is DateTime) bDate = bTs;

                        return bDate.compareTo(aDate);
                      });

                      if (docs.isEmpty) {
                        return const Padding(padding: EdgeInsets.all(12), child: Text('You have not raised any complaints yet.'));
                      }
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Text('ID: ${data['complaintId'] ?? ''} • Status: $status'),
                                const SizedBox(height: 6),
                                Text(data['description'] ?? ''),
                                const SizedBox(height: 4),
                                Text('Location: ${data['location'] ?? ''}', style: const TextStyle(fontSize: 12, color: Colors.black54)),
                              ]),
                              isThreeLine: true,
                              trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                                IconButton(icon: const Icon(Icons.edit), onPressed: () => _showEditDialog(doc)),
